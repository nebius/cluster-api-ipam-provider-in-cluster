...
func TestInClusterPrefixPoolDeleteValidation(t *testing.T) {
	...
	_, err := webhook.ValidateDelete(context.Background(), namespacedPool)
	g.Expect(err).To(HaveOccurred())
	_, err = webhook.ValidateDelete(context.Background(), globalPool)
	g.Expect(err).To(HaveOccurred())

	g.Expect(webhook.Client.(client.Client).DeleteAllOf(context.Background(), &ipamv1.IPAddress{})).To(Succeed())
...
func TestInClusterPrefixPoolCreateWarnsOn128WithGateway(t *testing.T) {
	...
	warnings, err := webhook.ValidateCreate(context.Background(), pool)
	g.Expect(err).NotTo(HaveOccurred())
...
func TestInClusterPrefixPoolUpdateWarnsOn128WithGateway(t *testing.T) {
	...
	warnings, err := webhook.ValidateUpdate(context.Background(), oldPool, newPool)
	g.Expect(err).NotTo(HaveOccurred())
	g.Expect(warnings).NotTo(BeEmpty())
	g.Expect(warnings).To(ContainElement(warnPrefixLength))
	g.Expect(warnings).To(ContainElement(warnGatewayCollision))
