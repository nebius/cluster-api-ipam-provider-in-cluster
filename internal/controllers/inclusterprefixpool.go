...
func genericPrefixReconcile(ctx context.Context, c client.Client, pool pooltypes.GenericInClusterPrefixPool) (_ ctrl.Result, reterr error) {
	...
	// NOTE: Prefix pool status semantics intentionally differ from legacy IP pool accounting.
	// OutOfRange tracks allocations that are currently non-allocatable under spec.
	// Free reports currently allocatable capacity, so out-of-range allocations are
	// added back when computing free.
