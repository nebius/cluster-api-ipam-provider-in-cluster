...
func (i *InClusterProviderAdapter) inClusterPrefixPoolToIPClaims(kind string) func(context.Context, client.Object) []reconcile.Request {
...
func (h *PrefixIPAddressClaimHandler) FetchPool(ctx context.Context) (client.Object, *ctrl.Result, error) {
	if h.claim.Spec.PoolRef.Kind == inClusterPrefixPoolKind {
		...
		h.pool = pool
	} else if h.claim.Spec.PoolRef.Kind == globalInClusterPrefixPoolKind {
...
func (h *PrefixIPAddressClaimHandler) EnsureAddress(ctx context.Context, address *ipamv1.IPAddress) (*ctrl.Result, error) {
	...
	if !allocated {
		...
		address.Spec.Prefix = ptr.To(int32(freePrefix.Bits()))
	}
	return nil, nil
}
